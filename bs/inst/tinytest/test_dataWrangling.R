...
app$set_inputs(`OP-editable_code` = paste0(app$get_values()$input[["OP-editable_code"]], " 2"))
app$click("OP-bracket_close")
app$click("OP-add")
app$click("OP-sin")
app$click("OP-colnames_conc_0")
...
content <- app$get_values()$input[["OP-editable_code"]]
expect_equal(
  gsub(" ", "", content), "conc^(2)+sin(conc)+cos(conc)+tan(conc)+sinh(conc)+cosh(conc))+tanh(conc))+asin(conc)+acos(conc)+atan(conc)"
)
iv_list <- app$get_values()$export[["OP-iv_list"]]
...
args <- list(
  "10000", "10000", "10000, 2", "10000, 10, 0.5"
)
